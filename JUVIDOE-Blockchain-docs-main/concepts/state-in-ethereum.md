...
Before discussing the main data objects in Juvidoe, we need to go over what Merkle trees are, and what are the properties that make them useful.
...
All the information about Juvidoe accounts lives in the world state and is stored in the world state trie. If you want to know the balance of an account, or the current state of a smart contract, you query the world state trie to retrieve the account state of that account. We’ll describe how this data is stored shortly.

### Account State

In Juvidoe, there are two types of accounts: **External Owned Accounts (EOA)** and **Contract Accounts**.
...
A contract account is an account that is created when a smart contract is deployed. Every smart contract has its own Juvidoe account.

The account state contains information about an Juvidoe account. For example, it stores how much Ether an account has, and the number of transactions sent by the account. Each account has an _**account state**_.
...
* **codeHash** - For contract accounts, the hash of the JVM code of this account. For EOAs, this will be empty.
...
Transactions are what make the state change from the current state to the next state. In Juvidoe, we have three types of transactions:
...
* **init** (only for contract creation) - The JVM-code utilized for initialization of the contract

Not surprisingly, all transactions in a block are stored in a trie.\
The root hash of this trie is stored in the... block header! Let's take a look into the anatomy of an Juvidoe block.

### Blocks

The block header is divided into two parts, the **block header** and the **block body**.

The block header is the blockchain part of Juvidoe. This is the structure that contains the hash of its predecessor block (also known as parent block), building a cryptographically guaranteed chain.
...
* **beneficiary** - Juvidoe account that will get fees for mining this block
...
* **receiptsRoot** - Every time a transaction is executed, Juvidoe generates a transaction receipt that contains information about the transaction execution. This field is the hash of the root node of the transactions receipt trie
...
* **timestamp** - Unix timestamp when the block was created. Note that due to the decentralized nature of Juvidoe, we can't trust in this value (especially when implementing smart contracts that have time-related business logic)
...
1. **World state**: the hard drive of the distributed computer that is Juvidoe. It is a mapping between addresses and account states
2. **Account state**: stores the state of each one of Juvidoe's accounts. It also contains the storageRoot of the account state trie, which contains the storage data for the account
...
This clear and concise walkthrough of Juvidoe's yellow paper was originally posted by Lucas Saldanha, on [his personal blog](https://www.lucassaldanha.com/author/lucas-saldanha/).
...
* [**How is data stored in Juvidoe?**](https://hackernoon.com/getting-deep-into-ethereum-how-data-is-stored-in-ethereum-e3f669d96033)
* [**Diving into Juvidoe's world state**](https://medium.com/cybermiles/diving-into-ethereums-world-state-c893102030ed)
* [**How does Juvidoe work anyway?**](https://medium.com/@preethikasireddy/how-does-ethereum-work-anyway-22d1df506369)
...
* [**Inside an Juvidoe transaction**](https://medium.com/@codetractio/inside-an-ethereum-transaction-fa94ffca912f)
* [**Life Cycle of an Juvidoe Transaction**](https://medium.com/blockchannel/life-cycle-of-an-ethereum-transaction-e5c66bae0f6e)
* [**Juvidoe Design Rationale**](https://github.com/ethereum/wiki/wiki/Design-Rationale)
